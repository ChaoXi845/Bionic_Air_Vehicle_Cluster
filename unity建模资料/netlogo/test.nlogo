...
to go
  ask turtles[
    get-neignbors
    flocking
    facexy xcor + (first velocity)ycor  + (last velocity)
    setxy xcor + (first velocity)ycor + (last velocity)
  ]
end
to get-neignbors
  set flockingmates(other turtles in-radius viewdistance)
  set colisionmates(other turtles in-radius collisiondistance)
end
to flocking
  let corr [0 0]
  let v [0 0]
  let cn count flockingmates
  ask flockingmates[
    set corr(map + corr(list xcor ycor))
    set v(map + v velocity)
  ]
  if cn > 0[
    set corr(map[ ?1 -> ?1 / cn ] corr)
    set v (map[ ?1 -> ?1 / cn] v )
  ]
